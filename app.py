...
with gr.Blocks(title="PrepGenie - Mock Interviewer") as demo:
    ...
    with gr.Column(visible=True) as main_app:
        ...
        mock_interview_btn.click(fn=navigate_to_interview, inputs=None, outputs=[interview_view, chat_view])
        if CHAT_MODULE_AVAILABLE:
            chat_btn.click(fn=navigate_to_chat, inputs=None, outputs=[interview_view, chat_view])
        history_btn.click(fn=navigate_to_history, inputs=None, outputs=[interview_view, chat_view])
